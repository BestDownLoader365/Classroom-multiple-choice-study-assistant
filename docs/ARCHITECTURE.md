...
`--skip-preflight` is the explicit, discouraged escape hatch. The gate consumes the *same frozen payload* through its in-memory entry point (`check_question_bank.main(argv, payload=...)`, and `check_glossary.main(argv, questions_payload=..., glossary_payload=...)` whose corpus is the frozen bank of the same run when `--questions` is part of it), never the candidate path: the path is passed only so the gate's report can name where the content came from, and both sides print the frozen digest. A candidate edited concurrently with a publish therefore cannot be validated as one revision and archived as another. Direct `cp` over a live file
...
The administrative namespace rename: it backs the database up (unless `--no-backup`) with the shared verified snapshot, verifies every precondition read-only *before* touching anything, rewrites `course_id` across every course-scoped table in one `BEGIN IMMEDIATE` transaction with before/after row-count validation and a full `PRAGMA foreign_key_check` that runs *inside* that transaction, before `COMMIT` (an explicit check, unaffected by the connection's `PRAGMA foreign_keys = OFF`), refuses to run when the target namespace already owns data, leaves `exam_questions` untouched (it follows its parent session), and updates the persisted `legacy_course_id` and — new — a `schema_meta.default_course_id` that pointed at the renamed course. Because the parent/child check is inside the transaction, a violation rolls the rename back and the refusal's "no rows were moved" wording is a fact rather than a post-commit finding.
...
dedicated connection
  ...
  BEGIN IMMEDIATE
    ...
    replace the old tables, then write schema_version + the persisted legacy_course_id
    PRAGMA foreign_key_check (full parent/child check, before COMMIT)
  COMMIT
  create the indexes (idempotent, retried by the next startup)
```

A failure *inside* the transaction rolls the whole thing back and the CLI reports the reason; re-running always reaches the same state. The full `PRAGMA foreign_key_check` runs *inside* that transaction, before `COMMIT` — `PRAGMA foreign_keys = OFF` does not affect it, because it is an explicit check — so a violation it reports rolls the layout back and its "No data was changed" message is a fact: the offending rows are still there to be repaired deliberately. A pre-existing parent/child violation in the old data aborts the migration with a report instead of silently dropping rows. Index creation is the one step left after the commit: `CREATE INDEX IF NOT EXISTS` is idempotent and validates nothing, so keeping it outside shortens the exclusive lock the row copy already holds, and a failure there leaves a consistent layout that the next startup repairs (every path through the migration calls it). A *fresh* database takes no transaction at all: there is nothing to copy and nothing to check, `_create_current_schema` writes the same empty layout through the same DDL, and every statement autocommits. The transaction itself never runs retention cleanup, weak-knowledge backfill, question-bank diffing, registry reconciliation or fingerprint recomputation — those are separate stages (`Database.enforce_attempt_retention`, `register_courses`, and each course's `QuestionBankSyncService.synchronize()` at startup).
...
3. Publish atomically: `python scripts/publish_course.py --course <course_id> --questions courses/<course_id>/questions_candidate.json` (paths resolve from the project root; omit `--questions` entirely when the working copy is the default one — `--course` itself is always required). The candidate bytes are frozen once into a payload, and that one payload is schema-validated, handed to the default gate (which reads the payload, not the path), written to an immutable `versions/<sha256>/questions.json`, re-validated against the publication baseline inside the course publication lock, and finally pointed at by the manifest through a single `os.replace()`. A candidate edited concurrently with a publish therefore cannot be preflighted as one revision and archived as another. Filesystem publication and database activation are **not** one transaction, so the command reports `published, pending worker activation` and never bumps the generation itself.
