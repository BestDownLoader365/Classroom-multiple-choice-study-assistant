"""Persistence for learner-scoped chapter reinforcement state."""
...
class WeakKnowledgePointRepository:
    """Store active weak chapters and distinct review verification IDs."""
    ...
    def activate_and_reset(
        ...
        with self.database.connect() as connection:
            connection.execute(
                ...
                """,
                (learner_id, chapter_id, timestamp, timestamp),
            )
        ...
        with self.database.connect() as connection:
            connection.execute(
                ...
                SET active = ?,
                    verified_question_ids = ?,
                    updated_at = ?
                WHERE learner_id = ? AND chapter_id = ?
                """,
                (
                    int(active),
    ...
    def delete_all_for_learner(self, learner_id: str) -> int:
        with self.database.connect() as connection:
            cursor = connection.execute(
                "DELETE FROM weak_knowledge_points WHERE learner_id = ?",
                (learner_id,),
            )
